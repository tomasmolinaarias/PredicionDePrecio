...
const urlApiProducts = "http://localhost:5000/api/v1/product";
  ...
  useEffect(() => {
    getAllProducts(urlApiProducts).then((data) => {
      ...
      const dataFilter = orderDate.filter(
        (element) => element.name.substring(0, 8).toUpperCase() === "TERCIADO"
      );
      ...
      const variationFirst = (
        ((numberOne - promedioTerciado) / promedioTerciado) *
        100
      ).toFixed(2);
      const numberTwo = Number(priceSecond.replace(/[$.]/g, "").trim());
      const variationSecond = (
        ((numberTwo - promedioTerciado) / promedioTerciado) *
        100
      ).toFixed(2);
